...
function OneItem() {
    ...
    useEffect(() => {
        itemDetail ? set_const_taken_units(itemDetail.taken_units) : set_const_taken_units(null)
    }, [const_taken_units])

    useEffect(() => {
        if (itemDetail) {
            set_taken_units_count(itemDetail.taken_units)
            itemDetail.taken_units < taken_units_count ?
                set_taken_units_count(taken_units_count) :
                set_taken_units_count(itemDetail.taken_units)
        }
    })
    ...
    return (
        <div>
            ...
            <button onClick={() => set_taken_units_count(taken_units_count - 1)}>-</button>
